...
for j in range(len(args.channel)):
    ...
    if args.trigger>=0:
        fig, ax = plt.subplots()
        ax.set_title('TT($V_p>3$mV) position distribution')
        h = ax.hist((info[j]['begin10']-triggerInfo['triggerTime'])[(info[j]['minPeak']>3)&(selectNearMax)],histtype='step', bins=np.arange(waveformLength-np.mean(triggerInfo['triggerTime'])), label='the largest peak pos')
        ax.set_xlabel('TT/ns')
        ax.set_ylabel('entries')
        ax.set_xlim([0, np.arange(waveformLength-np.mean(triggerInfo['triggerTime']))])
        ax.xaxis.set_minor_locator(MultipleLocator(10))
        pdf.savefig(fig)
        ax.set_yscale('log')
        pdf.savefig(fig)
        ...
        TTpeak = np.argmax(h[0])
        fig, ax = plt.subplots()
        ax.set_title('TT($V_p>3$mV) position distribution')
        h = ax.hist2d((info[j]['begin10']-triggerInfo['triggerTime'])[(info[j]['minPeak']>3)&(selectNearMax)], info[j]['minPeakCharge'][(info[j]['minPeak']>3)&(selectNearMax)], bins=[120,400], range=[[TTpeak-60, TTpeak+60],[0, 400]], cmap=cmap)
