...
if args.run == -1:
    ...
    with PdfPages(args.opt) as pdf:
        ...
        ax.set_xlabel('Gain in laser stage')
        ...
        ax.set_xlabel(r'$\tau_{\mathrm{ser}}$ in laser stage')
        ax.set_ylabel(r'$\sigma_{\mathrm{ser}}$ in laser stage')
        pdf.savefig(fig)
        plt.close()
        ...
        im = ax.pcolormesh(X, Y, resolutions, cmap=cm.gray)
