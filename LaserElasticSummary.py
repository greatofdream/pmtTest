...
for j in range(NChannels):
    ...
    # ax.fill_between([intervalCenters[j] - config.laserE/2, intervalCenters[j] + config.laserE/2], 0, 10, alpha=0.8)
    # ax.fill_between([intervalCenters[j] + config.elasticB, intervalCenters[j] + config.elasticE], 0, 10, alpha=0.8)
    ax.fill_between([intervalCenters[j]-3, intervalCenters[j] + 37], 0, 10, alpha=0.5, color='r', label='Delay time of elastically scattered $e^-$')
