...
if __name__=="__main__":
    ...
    with PdfPages(args.opt) as pdf:
        fig, ax = plt.subplots()
        ax.hist(prompt_ch2[prompt_ch2>-1], range=[100, 300], bins=50, histtype='step', label='ch2')
        ax.hist(prompt_ch3[prompt_ch3>-1], range=[100, 300], bins=50, histtype='step', label='ch3')
        ax.set_yscale('log')
        ...
        fig, ax = plt.subplots()
        ax.hist(delaypul1_ch2[delaypul1_ch2>-1], range=[600, 1300], bins=700, histtype='step', label='ch2')
        ax.hist(delaypul1_ch3[delaypul1_ch3>-1], range=[600, 1300], bins=700, histtype='step', label='ch3')
        ax.set_yscale('log')
        ax.set_xlabel('pulsePosition[ns]')
        ax.set_ylabel('entries')
        pdf.savefig(fig)
        ...
        fig, ax = plt.subplots()
        ax.hist(delaypul10_ch2[delaypul10_ch2>-1], range=[1300, 10300], bins=10000, histtype='step', label='ch2')
        ax.hist(delaypul10_ch3[delaypul10_ch3>-1], range=[1300, 10300], bins=10000, histtype='step', label='ch3')
        ax.set_yscale('log')
        ax.set_xlabel('pulsePosition[ns]')
        ax.set_ylabel('entries')
        pdf.savefig(fig)
